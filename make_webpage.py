...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Dragging States */
        .compare-col.is-dragging {
            opacity: 0.35;
            transform: scale(0.97);
        }
        .compare-col.drag-over {
            box-shadow: inset 0 0 0 3px var(--accent), 0 0 20px rgba(56, 189, 248, 0.4);
        }
        ...
        /* Sleek Minimalist Details Badge */
        .compare-col-details {
...
<script>
    ...
    function handleDragStart(e, index) {
        ...
        if (col) col.classList.add('is-dragging');
    }
    ...
    function handleDrop(e, targetIndex) {
        ...
        const movedItem = pinnedShades.splice(draggedIndex, 1)[0];
        pinnedShades.splice(targetIndex, 0, movedItem);
        draggedIndex = null;
        saveState();
        renderFloatingPill();
        renderCompareScreen();
    ...
    function handleDragEnd(e) {
        document.querySelectorAll('.compare-col').forEach(c => {
    ...
    function handleTouchMove(e) {
        ...
        const targetCol = elementUnderTouch ? elementUnderTouch.closest('.compare-col') : null;

        document.querySelectorAll('.compare-col').forEach(c => c.classList.remove('drag-over'));
        if (targetCol && targetCol.dataset.index != touchStartIndex) {
            targetCol.classList.add('drag-over');
            touchCurrentTarget = parseInt(targetCol.dataset.index);
        } else {
    ...
    function handleTouchEnd(e) {
        if (touchStartIndex !== null && touchCurrentTarget !== null && touchStartIndex !== touchCurrentTarget) {
            const movedItem = pinnedShades.splice(touchStartIndex, 1)[0];
            pinnedShades.splice(touchCurrentTarget, 0, movedItem);
            saveState();
            renderFloatingPill();
            renderCompareScreen();
        }
        document.querySelectorAll('.compare-col').forEach(c => c.classList.remove('is-dragging', 'drag-over'));
